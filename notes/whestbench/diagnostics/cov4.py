# The measurement says the whole error is the propagated sigma, and sigma^2(z_b) = sum_ac W W Sigma_ac
# is dominated by the OFF-DIAGONAL of the post-activation covariance.  Our off-diagonal closure is the
# exact Gaussian Hermite series plus a FIRST-ORDER third-cumulant term.  The next term in the cumulant
# expansion of E[F(z)] = E_G[F] + sum_k (1/k!) sum kappa_{i1..ik} E_G[d_{i1}..d_{ik} F] is, for
# F = relu(z_a) relu(z_b) and the index pattern (a,a,b,b) whose multiplicity is 4!/(2!2!) = 6,
#     dCov(h_a,h_b) = (1/4) kappa(z_a,z_a,z_b,z_b) E[relu''(z_a)] E[relu''(z_b)]
#                    = (1/4) kappa_aabb phi(t_a) phi(t_b) / (sigma_a sigma_b),
# because relu'' is a delta.  With the factorised ansatz kappa_aabb = g4 sigma_a^2 sigma_b^2 rho_ab^2
# this is  (g4/4) Sz_ab^2 F2_a F2_b  with F2 = phi/sigma -- the SAME outer structure the two-point
# term already uses, and computable elementwise from Sz.  n^2 flops: free.
# The analogous second-order third-cumulant term uses K21, which we also already have.
...
def predict(k,A4=0.0,p4=0.0,B3=0.0,form=2,sched=None,N=2400,seed=0):
    ...
    for l in range(1,L):
        ...
        if A4!=0.0:
            c4=A4*((l/15.0)**p4) if sched is None else A4*sched[l-1]
            if form==2:   Q=Sz*Sz                                   # kappa_aabb ~ g4 s^2 s^2 rho^2
            elif form==1: Q=np.abs(Sz)*so                           # ~ rho^1
            else:         Q=so*so                                   # ~ rho^0  (rank one)
            Cn=Cn+(c4*Q*np.outer(F2,F2)).astype(np.float32)
        if B3!=0.0:
            Cn=Cn+(B3*(K21*K21.T)*np.outer(F2,F2)).astype(np.float32)
...
if __name__=="__main__":
    ...
    print(f"{'form':>6} {'A4':>10} {'p4':>5}  {'fit':>11} {'ratio':>8}")
    best=(b,{})
    GRID={2:(0.2,0.6,2.0,6.0,20.0),1:(0.002,0.006,0.02,0.06,0.2),0:(0.0002,0.0006,0.002,0.006,0.02)}
    for form,gs in GRID.items():
        for A4 in gs:
            for p4 in (0.0,) if form!=2 else (0.0,-1.0):
                f=sc(FIT,A4=A4,p4=p4,form=form)
                if f<best[0]: best=(f,dict(A4=A4,p4=p4,form=form))
                print(f"{form:6d} {A4:10.4f} {p4:5.1f}  {f:11.4e} {f/b:8.4f}", flush=True)
    print(f"\nbest {best[1]}  fit {best[0]:.4e}")
    if best[1]:
        print(f"held-out(8-15): deployed {bh:.4e}   with term {sc(HOLD,**best[1]):.4e}")
