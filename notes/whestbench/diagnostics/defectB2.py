...
print("Exactly zero for the true functional.  Reported BOTH ways: relative to ||M|| (as originally")
print("recorded) and relative to the larger of the two RESPONSE norms (as the Mac report does).\n")
print(f"{'start':>5} {'||dS||':>10} {'||d2/2||':>10} | {'closure /||M||':>14} {'closure /resp':>13}"
      f" | {'truth /||M||':>12} {'truth /resp':>11}")
for START in (4,8,11,13):
    ...
    Pt=tdS-0.5*td2
    rc=max(np.linalg.norm(dS),np.linalg.norm(0.5*d2))
    rt=max(np.linalg.norm(tdS),np.linalg.norm(0.5*td2))
    print(f"{START:5d} {np.linalg.norm(dS):10.3e} {np.linalg.norm(0.5*d2):10.3e} |"
          f" {np.linalg.norm(Pc)/np.linalg.norm(base):14.3e} {np.linalg.norm(Pc)/rc:12.2%}"
          f" | {np.linalg.norm(Pt)/np.linalg.norm(tb):12.3e} {np.linalg.norm(Pt)/rt:10.2%}", flush=True)
