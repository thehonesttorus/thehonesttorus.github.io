# Exact k3 at every layer buys only 31%.  So the residue is not the cubic channel.  Decompose it with
# the NOISE-FREE ground truth: mu_l = E[z_l] = Y_{l-1} W_l exactly, and Y_l is known to 7.5e-11.
# Substituting the true layer mean at every step costs nothing in Monte-Carlo noise and isolates the
# per-step error from its accumulation.
...
def predict(k,oracle_mean=None,oracle_k3=None,oracle_k4=None,lam=None,seed=0,N=2400):
    ...
    if oracle_mean is not None and oracle_mean(0): m=Y[0].astype(np.float32)
    for l in range(1,L):
        ...
        k4v=g4*s4
        if oracle_k3 is not None and oracle_k3(l): k3=lam[3][l]*s3
        if oracle_k4 is not None and oracle_k4(l): k4v=lam[4][l]*s4
        muh=mu/np.linalg.norm(mu)
        Ap=zc@muh.astype(np.float32); u_mc=((zc*zc).T@Ap).astype(np.float64)/N-K21@muh
        u=0.75*(C_S[l-1]*s3)+0.25*u_mc
        ...
        m=m_next.astype(np.float32)
        if l<L-1 and oracle_mean is not None and oracle_mean(l): m=Y[l].astype(np.float32)
    return np.mean((m.astype(np.float64)-Y[-1])**2)

NETS=[0,1]; LAM={}
for k in NETS:
    d=np.load(f"rw_{k}_{NB}.npz"); Hm=d['Hm']; LAM[k]={3:Hm[:,3],4:Hm[:,4]}
ALL=lambda l: True
CFG=[("deployed",                        dict()),
     ("exact mean, every layer",         dict(oracle_mean=ALL)),
     ("exact mean, layers >= 8",         dict(oracle_mean=lambda l: l>=8)),
     ("exact k3, every layer",           dict(oracle_k3=ALL)),
     ("exact mean + k3, every layer",    dict(oracle_mean=ALL,oracle_k3=ALL)),
     ("exact mean + k3 + k4, every layer",dict(oracle_mean=ALL,oracle_k3=ALL,oracle_k4=ALL))]
print("Noise-free mean oracle (the layer means are ground truth at 1e9 samples).  Final-layer MSE.\n")
print(f"{'configuration':36s}  " + "  ".join(f"net {k}" for k in NETS) + "    mean      /deployed")
base=None
for name,kw in CFG:
    v=[predict(k,lam=LAM[k],**kw) for k in NETS]; mv=float(np.mean(v))
    if base is None: base=mv
    print(f"{name:36s}  " + "  ".join(f"{x:.3e}" for x in v) + f"   {mv:.3e}  {mv/base:8.4f}", flush=True)
