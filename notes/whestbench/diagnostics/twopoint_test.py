# Ship the two-point kernel c_l * sigma^3 (x) muhat in place of, or blended with, the Monte-Carlo
# residual estimate.  Schedule fitted on networks 0-7; evaluated on held-out networks 8-15.
...
def predict(k, mode='mc', N_MC=2400, seed=0, blend=0.5):
    ...
    for l in range(1,L):
        ...
        z=Zs[l].astype(np.float64); a=z.mean(0); zc=z-a; v=(zc*zc).mean(0); k3s=(zc**3).mean(0)
        ...
        u_mc=(zc*zc).T@Ap/N_MC-K21m@muh
        u_sh=C2P[l-1]*s3
        if   mode=='mc':    u=u_mc
        elif mode=='ship':  u=u_sh
        elif mode=='blend': u=blend*u_sh+(1-blend)*u_mc
        elif mode=='none':  u=np.zeros(n)
        K21=K21m+np.outer(u,muh); np.fill_diagonal(K21,0.0)
        F2=ph/sz
...
print("Held-out networks 8-15, final-layer MSE.  Two-point mean-direction coefficient:\n")
print(f"{'mode':44s}   mean         per network")
for name,kw in [("none (model two-point only)",       dict(mode='none')),
                ("Monte-Carlo residual (deployed)",   dict(mode='mc')),
                ("shipped c_l * sigma^3",             dict(mode='ship')),
                ("blend 0.25 shipped / 0.75 MC",      dict(mode='blend',blend=0.25)),
                ("blend 0.50 shipped / 0.50 MC",      dict(mode='blend',blend=0.50)),
                ("blend 0.75 shipped / 0.25 MC",      dict(mode='blend',blend=0.75))]:
    v=[predict(k,**kw) for k in range(8,16)]
    print(f"{name:44s} {np.mean(v):.3e}   " + " ".join(f"{x:.2e}" for x in v), flush=True)
